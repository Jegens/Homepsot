...
class Love extends Db {
    ...
    function add($love_data){
        ...
        $sql = "SELECT * FROM loves WHERE listing_id = '$listing_id' AND user_id = '$user_id'";
        $love = $this->select($sql)[0];

        if(!empty($love['id'])){ // already loved, delete the love
            $sql = "DELETE FROM loves WHERE listing_id = '$listing_id' AND user_id = '$user_id'";
            $this->execute($sql);
            $love_data['loved'] = 'unloved';
            $love_data['error'] = false;
        }else{
            $sql = "INSERT INTO loves (listing_id, user_id) VALUES('$listing_id', '$user_id')";
        ...
        $sql = "SELECT COUNT(id) AS love_count FROM loves WHERE listing_id = '$listing_id'";
